...
Backbone.sync = function(method, model, options, error) {
	...
	if(method == 'update')
	{
		options.url = options.url + '&amp;todo:method=PUT';
	}
	else if(method == 'delete')
	{
		options.url = options.url + '&amp;todo:method=DELETE&amp;todo:ids=' + model.get('id');
	}
	else if(method == 'updateList')
	{
		method = 'update';
		options.url = options.url + "&amp;todo:method=PUTLIST";
	}
	else if(method == 'deleteList')
	{
		options.url = options.url + "&amp;todo:method=DELETE&amp;todo:ids=";
		for(var i = 0; i < model.models.length; i++){
			if(model.models[i].get('completed') == true) {
				options.url += model.models[i].get('id') + '|';
			}		
		}
